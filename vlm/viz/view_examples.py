...
# This script lives in vlm/viz/; the result files live one level up in vlm/result/.
RESULT_ROOT = os.path.join(os.path.dirname(VLM_DIR), "result")
...
def load_solver_prompts(result_dir):
    ...
    Solver run files are any *.json in the dir that are not verifier files and not the
    scores/prompts sidecars -- works for any dataset (countbench_*, charxiv_*, ...).
    """
    prompts = {}
    for f in glob.glob(os.path.join(result_dir, "*.json")):
        base = os.path.basename(f)
        if base.startswith("verify_") or base.endswith(("_scores.json", "_prompts.json")):
            continue
        d = json.load(open(f))
...
def main():
    ...
    ap.add_argument("--result_dir", default=RESULT_ROOT)
