...
# Resolve every solver's base scored file once.
SOLVER_FILES=(); MISSING=()
for M in $GRID_MODELS; do
  f=$(base_file "$(short "$M")")
  if [[ -n "$f" ]]; then SOLVER_FILES+=("$f"); else MISSING+=("$M"); fi
done
...
for V in $GRID_MODELS; do
  VS=$(short "$V")
  have=$(ls "$OUTDIR"/verify_${DS}_solver-*_verifier-"${VS}"_*.json 2>/dev/null | wc -l)
  if [[ "$have" -ge "$NSOLVERS" ]]; then log "SKIP verifier=$VS (all $NSOLVERS pairs exist)"; NSKIP=$((NSKIP+1)); continue; fi
  LOGF="$LOGDIR/vg_${DS}_verifier-${VS}.log"
  wait_gpu || true
  log "START verifier=$VS judging $NSOLVERS solver runs"
  if $PY vlm/vlm_verify.py --verifier_model_name "$V" $(extra_for "$V") $LIMARG \
        --solver_run_files "${SOLVER_FILES[@]}" --output_dir "$OUTDIR" >"$LOGF" 2>&1; then
    log "OK    verifier=$VS"; NDONE=$((NDONE+1))
  else
    log "FAIL  verifier=$VS (exit $?) -- see $LOGF"; NFAIL=$((NFAIL+1))
